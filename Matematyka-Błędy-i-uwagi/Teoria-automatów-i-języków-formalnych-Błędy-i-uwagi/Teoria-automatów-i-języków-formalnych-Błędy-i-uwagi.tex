...
\noindent
\Str{7} Tak jak w~przypadku symbolu $\mod_{ 6 }$,
wydaje mi~się, że~lepiej wyglądałby symbol $\Ker_{ h }$.
...
\noindent
\StrWierszDol{7}{15} Użyty w~podanej tu formule $\rho \subset S^{ 2 }$
...
\StrWierszDol{11}{11} Według mnie zapis $\min\{ k, l \}$ wyglądałoby znacznie
...
\StrWierszGora{13}{8} Nie rozumiem czemu w~tej linii wprowadzono
...
\StrWierszGora{14}{12} Odstępy w~tej linii są stanowczo za~duże.
...
\StrWierszDol{14}{9} Odstęp nad tą linią jest zbyt duży.
...
\StrWierszDol{20}{10} W~tym miejscu, tak ja na stronie~7, symbol
...
\StrWierszGora{21}{7} W~tej linii znajdujemy tajemniczo wyglądający wzór
\begin{equation}
...
\PowinnoByc A.A. Milne \\
\textbf{Str. 8, pierwszy rysunek.} \\
\Jest $S /_{ Ker_{ h } }$ \\
\PowinnoByc $S_{ / Ker_{ h } }$ \\
\textbf{Str. 8, drugi rysunek.} \\
\Jest $A^{ * } /_{ Ker_{ h } }$ \\
\PowinnoByc $M_{ / Ker_{ h } }$ \\
\StrWierszGora{10}{3} \\
\Jest \textit{to znaczy} $M = B^{ * }$ \\
\PowinnoByc czyli $M$ jest kanonicznie izomorficzny z~pewnym wolnym monoidem
słownym~$B^{ * }$, \\
\StrWierszDol{10}{23} \\
\Jest \textit{Homomorfizm $h$ jest, jak łatwo stwierdzić, izomorfizmem,
  więc monoid $M$ jako izomorficznym z~$A^{ * }$ jest wolny.} \\
\PowinnoByc Ponownie, z~faktu że każdy element $m \in S$ można rozłożyć na
...
\Jest \textit{Ponieważ, jak uzasadniliśmy, grupa jedynki jest
  trywialna,} \\
\PowinnoByc Ponieważ $S$ jest półgrupą, wynika stąd,~że \\
