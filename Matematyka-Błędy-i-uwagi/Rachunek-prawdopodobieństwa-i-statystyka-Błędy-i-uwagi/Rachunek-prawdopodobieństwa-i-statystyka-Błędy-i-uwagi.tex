...
\noindent
\Str{35} Autor rozpoczyna tu dyskusję pojęcia \textit{zdarzenia losowego}
i~\textit{zdarzenia elementarnego}, które to są fundamentalne dla~całego
...
przejdziemy, warto zaznaczyć, że~pojęcie „zdarzenia” i~„zdarzenia losowego”
...
nadzieję, że~będzie je cechował wysoki poziom ścisłości. Wybieramy takie
podejście, gdyż naszym celem nie jest po prostu wprowadzenie matematycznych
pojęć, lecz wyjaśnienie czemu je wprowadzamy i~jaki jest ich ogólny sens.
Dlatego też naszym punktem wyjścia jest omówienie omówienia problemu, który
przenika cały rachunek prawdopodobieństwa.

Problem ten jest następujący. Mamy doświadczenie, które jest powtarzalne
i~zawsze produkuje jeden
konkretnych wynik. Na początek przyjmijmy, że~dane doświadczenie może zwrócić
tylko skończoną liczbę wyników $e_{ 1 }$, $e_{ 2 }$, \ldots, $e_{ n }$. Jest to
uzasadnione tym, że~zrozumienie wariantu problemu, gdy liczba wyników jest
...
czarnych i~ponumerowanych liczbami od $1$ do $7$. Doświadczenie będzie
polegało na wyciągnięciu jednej kuli z~urny, bez zaglądania do wnętrza urny,
...
zaś jako synonim „zdarzenia losowego”. Zauważmy bowiem, że w~problemach
...
elementarny. Zaś bez zidentyfikowania zdarzeń elementarnych nie jesteśmy
...
w~danym eksperymencie zaszło wybrane zdarzenie, czy nie?
...
w których biorą udział, to zupełnie inne „obiekty”. Niemniej zdarzenie
...
korzystać z~tego typu skrótów myślowych, jeśli odpowiednio uprości to nasze
rozważania.
...
sposób sformułować w~języku teorii mnogości. Oznaczmy bowiem
...
Mamy powtarzalne doświadczenie, w~wyniku wykonania którego daje nam jeden
określony rezultat, który nazywamy zdarzeniem elementarnym. Założyliśmy,
...
z~powrotem do urny, z~której ponownie wyciągamy kulę. Pytamy~się, czy za
pierwszym razem wyciągnęliśmy białą kulę z~numerem $1$, a~za drugim razem
...
W~omawianej książce tego typu ciągi nazywane są \textit{kombinacjami},
...
czy zaszły one i~to w~zadanej kolejności.

Problem ten natychmiast~się uogólnia. Rozpatrzmy dwa zdarzenia
...
Zanim przejdziemy dalej, zauważmy, że~problemy te można teraz uogólniać na
wiele sposobów. Po pierwsze możemy, mieć dwa doświadczenia, oznaczmy je
$D_{ 1 }$ i~$D_{ 2 }$, zaś ich przestrzenie zdarzeń elementarnych przez
$E_{ 1 }$ i~$E_{ 2 }$. Możemy teraz powstaje pytanie, czy wykonując najpierw
doświadczenie, a~potem drugie, dostaniemy ciąg wyników $a_{ i }$,
$i \in \{ 1, 2 \}$, taki że $a_{ i } \in A_{ i }$?
...
zwykle nie przeszkadzają nam rozwiązywać zadań.
