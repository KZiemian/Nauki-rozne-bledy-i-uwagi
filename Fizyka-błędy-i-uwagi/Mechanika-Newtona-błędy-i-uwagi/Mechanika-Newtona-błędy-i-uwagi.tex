...
Niech teraz będą dane $u_{ \HorSpaceOne 1 }, u_{ \HorSpaceOne 2 } \in U$. Każdej
parze takich przestrzeni przyporządkowujemy pewien rodzaj iloczynu, który
będziemy oznaczać
...
odwzorowaniem dwuliniowym. Aby dookreślić te odwzorowania w~każdej
przestrzeni $V_{ u }$ wybieramy w~dowolny sposób jeden niezerowy wektor,
oznaczmy go~$e_{ \HorSpaceTwo u }$, i~kładziemy
\begin{equation}
  \label{eq:Mechanika-Rozwazania-ogolne-08}
  \psi_{ \HorSpaceOne u_{ \HorSpaceOne 1 }, \, u_{ \HorSpaceOne 2 } }(
  e_{ \HorSpaceTwo u_{ \HorSpaceOne 1 } }, e_{ \HorSpaceTwo u_{  \HorSpaceOne2 } } ) =
  e_{ \HorSpaceTwo u_{ \HorSpaceOne 1 } u_{ \HorSpaceOne 2 } }.
\end{equation}
Wprowadzamy również operację dzielenia elementu z~$V_{ u_{ 1 } }$ przez
niezerowy element z~przestrzeni $V_{ u_{ 2 } }$:
\begin{equation}
  \label{eq:Mechanika-Rozwazania-ogolne-09}
  \varphi_{ \HorSpaceOne u_{ \HorSpaceOne 1 }, \, u_{ \HorSpaceOne 2 } } :
  V_{ u_{ \HorSpaceOne 1 } } \times ( V_{ u_{ \HorSpaceOne 2 } } \setminus \{ 0 \} ) \to
...
\begin{subequations}
  \begin{align}
    ...
    \varphi_{ \HorSpaceOne u_{ 1 }, \, u_{ 2 } }( \alpha \, v_{ 1 } + \beta \, v_{ 2 }, w )
    &= \alpha \HorSpaceOne \varphi_{ \HorSpaceOne u_{ 1 }, \, u_{ 2 } }( v_{ 1 }, w )
      + \beta \HorSpaceOne \varphi_{ \HorSpaceOne u_{ 1 }, \, u_{ 2 } }( v_{ 2 }, w ), \\
    \label{eq:Mechanika-Rozwazania-ogolne-10-B}
    \varphi_{ \HorSpaceOne u_{ 1 }, \, u_{ 2 } }( v, \lambda \HorSpaceOne w )
    &= \frac{ 1 }{ \lambda } \, \varphi_{ u_{ 1 }, \, u_{ 2 } }( v, w ), \\
    \label{eq:Mechanika-Rozwazania-ogolne-10-C}
    \varphi_{ \HorSpaceOne u, \, u }( v_{ u }, v_{ u } ) &= I \in V_{ I },
  \end{align}
\end{subequations}
...
wektorów przez liczby w~przestrzeniach wektorowych, możemy udowodnić wiele
innych własności funkcji $\psi_{ u_{ 1 }, \, u_{ 2 } }$ i~$\varphi_{ u_{ 1 }, \, u_{ 2 } }$,
analogicznych dla mnożenia i~dzielenia liczb rzeczywistych.
Ponieważ metoda dowodzenia ich powinna być w~tym momencie jasna, nie
będziemy~się zagłębiać w~szczegółowe dowody, chyba, że~zmuszą nas do~tego
konkretne problemy.

Przedstawiony powyżej schemat powinien nam pozwolić opisać wszystkie
podstawowe operacje wykonywane na~wielkościach fizycznych posiadających
jednostki, co~zilustrujemy kilkoma przykładami. Na początku wybieramy
odpowiednie bazy w~przestrzeniach $V_{ L }$, $V_{ T }$ i~$V_{ M }$. Niech
tymi bazami będą $e_{ \si{m} } \in V_{ L }$, $e_{ \si{s} } \in V_{ T }$,
$e_{ \si{kg} }$, gdzie wektor $e_{ \si{m} }$ będziemy interpretować jako
oznaczający metr, $e_{ s }$ jako sekundę, a~$e_{ \si{kg} }$ jako oznaczający
kilogram. Jeżeli teraz mamy dwa wektory $v_{ 1 }, v_{ 2 } \in V_{ L }$,
$v_{ 1 } = 10 \, e_{ \si{m} }$, $v_{ 2 } = 5 \, e_{ \si{m} }$, to
\begin{equation}
...
\begin{equation}
  \label{eq:Mechanika-Rozwazania-ogolne-21}
...
jako metr nad sekundę i~zapisywali za pomocą standardowej notacji
jako
\begin{equation}
  \label{eq:Mechanika-Rozwazania-ogolne-23}
  \varphi_{ L, \, T }( e_{ \si{m} }, e_{ \si{s} } ) = \frac{ \si{m} }{ \si{s} }.
\end{equation}
Wykazane powyżej „prawa łączności i~rozdzielności”
gwarantują, że~otrzymane wyniki nie zależą od~kolejności wykonywania
działań, tak jak tego oczekiwaliśmy.

Zauważmy, że~w~obecnym formalizmie, zmiana jednostek danej wielkości
fizycznej sprowadza~się do zmiany bazy w~przestrzeniach $V_{ L }$, $V_{ T }$
i~$V_{ M }$. Przykładowo zmiana jednostki czasu z~sekund na minuty
sprowadza~się do przejścia do bazy
\begin{equation}
  \label{eq:Mechanika-Rozwazania-ogolne-24}
  e_{ \si{min} } = 60 \, e_{ \si{s} }, \quad
  e_{ \si{s} }   = \frac{ 1 }{ 60 } e_{ \si{min} }.
\end{equation}

Podane wyżej rozważania dotyczyły wielkości które mogą być reprezentowane,
przez jedną liczbę rzeczywistą wraz z~jednostkami fizycznymi, zwyczajowo
wielkości takie określa~się jako „wielkości skalarne” lub „skalary”. Nie
obejmują one choćby taki obiektów fizycznych posiadających jednostki, jak
wektor pola elektrycznego $\vecE( \vecx, t )$ w~chwili $t$ i~w~punkcie
$\vecx$, jednak uogólnienie go na takie przypadki nie powinno stanowić
problemu, dlatego je pominiemy. Jeśli problemy które napotkamy będą
wymagały pewnych zmian lub uogólnień w~przedstawionym tu formalizmie,
będziemy~się starali do~niego powrócić i~zmodyfikować w~odpowiedni sposób.
