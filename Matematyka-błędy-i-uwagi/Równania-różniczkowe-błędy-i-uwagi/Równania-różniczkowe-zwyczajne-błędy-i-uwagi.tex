...
notatkach będziemy~się starać zawsze podawać jawnie argumenty od których
zależą funkcje, by uniknąć możliwych nieporozumień.
...
Pozostaje pytanie, w~jaki sposób określamy dziedzinę rozwiązania, czyli
zbór~$A_{ 1 }$? Standardowym wyborem jest przyjęcie, że~wybieramy jako
$A_{ 1 }$ największy odcinek otwarty $( a, b ) \subset A_{ 0 }$ w~którym jesteśmy
w~stanie zdefiniować rozwiązanie równania. Co jednak zrobić w~przypadku
równań takich jak równanie (?) ze strony (?), które posiada rozwiązanie
postaci
\begin{equation}
...
\eqref{eq:Matwiejew-Metody-calkowania-ETC-08} jak i~dobrze znaną zależność
\begin{equation}
  \label{eq:Matwiejew-Metody-calkowania-ETC-01}
  y'( x ) =
...
\eqref{eq:Matwiejew-Metody-calkowania-ETC-08}
