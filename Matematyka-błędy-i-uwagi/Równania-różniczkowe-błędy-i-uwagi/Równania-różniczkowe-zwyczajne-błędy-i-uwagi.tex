...
kierunków w~tym fragmencie notatek będziemy rozpatrywać tylko
w~najprostszym przypadku, tym samym który jest omówiony w~książce, czyli
gdy szukana funkcja jest odwzorowaniem $y : A \to \Rbb$,
$A \subset \Rbb$, a~równanie które ma spełniać jest standardowej postaci:
\begin{equation}
  \label{eq:Matwiejew-Metody-calkowania-ETC-41}
  y'( x ) = f\big( x, y( x ) \big).
\end{equation}
W~związku z~tym pole kierunków jest zdefiniowane na
płaszczyźnie $xy$, gdzie $x$ odpowiada zmiennej niezależnej, a~$y$
wartościom przyjmowanym przez funkcję $y$.
...
przypadkach mamy $A = ( t_{ 0 }, t_{ 1 } )$. Nie ma to wielkiego znaczenia
dla ogólności przedstawionych rozważań. Niech szukana funkcja będzie postaci
$y : ( t_{ 0 }, t_{ 1 } ) \to \Rbb$. Wówczas krzywa całkowa
...
całkowa, dana przez funkcję $y_{ 1 }$. Równaniem tej krzywej jest postaci
$x \mapsto \big( x, y( x ) \big)$, więc wektor $\vecv$ styczny do~niej w~tym
punkcie jest dany przez
\begin{equation}
...
wektorowe zdefiniowane wzorem \eqref{eq:Matwiejew-Metody-calkowania-ETC-46}.
