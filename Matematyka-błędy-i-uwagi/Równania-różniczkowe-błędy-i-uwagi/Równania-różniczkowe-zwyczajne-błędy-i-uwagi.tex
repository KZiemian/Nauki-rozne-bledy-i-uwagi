...
największym odcinkiem otwarty zawartym w~zbiorze $A_{ 0 }$, w~którym
jesteśmy w~stanie zdefiniować rozwiązanie równania
\eqref{eq:Matwiejew-Metody-calkowania-ETC-10}. Co jednak zrobić w~przypadku
...
\eqref{eq:Matwiejew-Metody-calkowania-ETC-12} jest ona równa $\Rbb^{ 3 }$.
Łatwo~się też przekonać, że~rozwiązaniem równania
\eqref{eq:Matwiejew-Metody-calkowania-ETC-13} jest funkcja
...
\eqref{eq:Matwiejew-Metody-calkowania-ETC-18}. Ponieważ dziedzina $F$ jest
...
tej funkcji do \eqref{eq:Matwiejew-Metody-calkowania-ETC-21} prowadzi do
...
\noindent
\Str{27--28} Na stronie tych znajdujemy definicję, która określa kiedy
...
\eqref{eq:Matwiejew-Metody-calkowania-ETC-73}. Po~jego lewej stronie mamy
...
\eqref{eq:Matwiejew-Metody-calkowania-ETC-75} jest rozwiązaniem równania
\eqref{eq:Matwiejew-Metody-calkowania-ETC-74} dla dowolnej wartości
stałej~$C$. Tym samym dowolny związek postaci $C = \psi( x, y )$, przy
założeniu, że~prowadzi on do~$C$ rzeczywistego, daje jako wynik stałą taką,
że~funkcja $y( x ) = C x$ jest rozwiązaniem rozpatrywanego równania. Z~tego
punktu widzenia nie ma nic niezwykłego w~związku
\eqref{eq:Matwiejew-Metody-calkowania-ETC-76}.

Tym co wyróżnia \eqref{eq:Matwiejew-Metody-calkowania-ETC-76} spośród
...
\begin{equation}
  ...
  y( x_{ 0 }, C_{ 0 } ) = y_{ 0 }.
\end{equation}
Fakt, że~funkcja $\psi( x, y )$ musi spełniać ten warunek, by $y( x ) = C x$
było rozwiązaniem ogólnym badanego równania, jest zawarty w~definicji tej
funkcji podanej na~stronie~27, gdzie jest napisane, że~ma otrzymujemy ją poprzez odwikłanie równania
\begin{equation}
  \label{eq:Matwiejew-Metody-calkowania-ETC-79}
  y = \varphi( x, C ),
\end{equation}
na zmienną $C$. Wzór \eqref{eq:Matwiejew-Metody-calkowania-ETC-79} należy
rozumieć jako równanie uwikłane uwzględniające trzy zmienne niezależne:
$x$, $y$ i~$C$. Aby ta procedura miała sens funkcja $\varphi( x, C )$ musi
spełniać wszystkie założenia podane na~stronie~27.

{\Large Od strony 29 należy zacząć ponowne czytanie książki.}
