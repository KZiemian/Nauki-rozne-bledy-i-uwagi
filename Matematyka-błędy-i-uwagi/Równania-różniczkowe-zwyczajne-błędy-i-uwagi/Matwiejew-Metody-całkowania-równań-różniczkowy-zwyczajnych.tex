...
przypadków jeżeli jest to możliwe, to będziemy szukali funkcji określonej
na zbiorze $A = ( a, b )$, dopuszczamy przy tym sytuację, że $a$ i~$b$ mogą
...
postaci gdy $A = \bigcup_{ i = 1 }^{ n } ( a_{ i }, b_{ i } )$. Z~tych samych
...
Pierwszy raz równanie, które wymaga szukania rozwiązania o~takiej dziedzinie
napotykamy na stronie 15. Podane tam równanie $y'( x ) = y( x )^{ 2 }$
...
Przejdziemy teraz do podania definicji równania różniczkowego zwyczajnego.
...
\eqref{eq:Czym-jest-rownanie-ETC-06} jest szczególnym przypadkiem
równania \eqref{eq:Czym-jest-rownanie-ETC-04}, wiele z~tego co mówiliśmy
o~funkcji~$F$ przenosi~się bez większych zmian na funkcję~$f$.
Przy analizie funkcji~$f$ będziemy też stosować te same zasady co dla
funkcji~$F$, chyba że~powiedziano inaczej. Do~problemu określenia dziedziny
funkcji $F$ i~$f$ powrócimy w~dalszym ciągu notatek, teraz zaś
poprzestaniemy na~zauważeniu, że~funkcja~$F$ podana wyżej w~oczywisty
sposób zależy istotnie od~$y^{ ( n ) }( x )$.
