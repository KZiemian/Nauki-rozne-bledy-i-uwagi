...
\start \Str{32} Napisać więcej o wzorze ????
\begin{equation*}
  \Omega_{ l m } = \omega( e_{ l }, e_{ m } ).
\end{equation*}
...
\begin{subequations}
  \begin{align}
    \label{eq:Wojtynski-09-A}
    &d'_{ i } = d_{ j }, \\
    \label{eq:Wojtynski-09-B}
    &d'_{ j } = d_{ i }, \\
    \label{eq:Wojtynski-09-C}
    &d'_{ l } = d_{ l },
  \end{align}
\end{subequations}
...
\begin{equation}
  \label{eq:Wojtynski-15}
  \Omega'_{ j i } = \Omega_{ i j }.
\end{equation}
Wzory \eqref{eq:Wojtynski-12}--\eqref{eq:Wojtynski-15} stanowią indeksowy
zapis reguł 1 i~2.
...
\begin{subequations}
  \begin{align}
    \label{eq:Wojtynski-16-A}
    d_{ 2 } &\leftrightarrow d_{ k + 1 }, \\
    \label{eq:Wojtynski-16-B}
    d_{ 2 } &\leftrightarrow d_{ 3 }, \\
    \label{eq:Wojtynski-16-C}
    d_{ 3 } &\leftrightarrow d_{ 4 }, \\
    \label{eq:Wojtynski-16-D}
    &\hspace{0.65em} \vdots \\
    \label{eq:Wojtynski-16-E}
...
dzięki wzorom \eqref{eq:Wojtynski-12}--\eqref{eq:Wojtynski-15}, poprzez
dokładną analizę, będziemy w~stanie pokazać, że~ciąg zmian baz
\eqref{eq:Wojtynski-16-A}--\eqref{eq:Wojtynski-16-E} prowadzi od macierzy
...
\eqref{eq:Wojtynski-16-A}--\eqref{eq:Wojtynski-16-E} dla przypadku $n = 8$,
...
\start \Str{34} Lepiej wyjaśnić zdanie, że forma symplektyczna to taka,
która ma postać (2.11). ???
...
\begin{center}
  ...
  \begin{tabular}{|c|c|c|c|c|}
    ...
    % & & &  &  \\
    % & &  & $U \subset \Omega_{ x_{ 0 } }$ & $U \in \Omega_{ x_{ 0 } }$ \\
    %   & & & $\rho( X, Y )$ & $\rho( x, y )$ \\
    %   & &  & zbioru & niepustego zbioru \\
    %   & & & $y_{ \varphi } \in X_{ \varphi }$ & $x_{ \varphi } \in X_{ \varphi }$ \\
    %   & & & $x_{ 1 },\;\; x_{ 2 } \in X$ & $x_{ 1 },\, x_{ 2 } \in X$ \\
    %   & & & $x e_{ + }$ & $x \, e_{ + }$ \\
    %   & & & $\mu\big( x, \mu( x, y ) \big)$
    %        & $\mu\big( x, \mu( y, z ) \big)$ \\
    %   & & & $\mathbf{x} \in \mathbf{X}$ & $x \in X$ \\
    %   & & & $\mu \omega( x_{ 1 }, y )$ & $\mu \, \omega( x_{ 2 }, y )$ \\
    %   &  & & $W$ modulo $V$, & $W$, \\
    %   &  & & $\tau( x_{ j }, y_{ k } )$
    %        & $\tau( x_{ j }, y_{ k } ) = \pi( x_{ j } \otimes y_{ k } )$ \\
    %   & & & $X^{ k } \times X^{ m }$ & $X^{ \otimes k } \times X^{ \otimes m }$ \\
    %   & & & $X^{ k } \otimes X^{ m }$ & $X^{ \otimes k } \otimes X^{ \otimes m }$ \\
    %   &  & & $\overline{ F( y, x ) }$ & $\overline{ F( y, x ) }$. \\
    %   &  & & (tj. & Tj. \\
    %   &  & & $\Rbb$)) & $\Rbb$) \\[0.3em]
    %   &  & & $\displaystyle
    %              \frac{ \overline{ \langle x, y \rangle } }{ | \langle x, y \rangle | }$
    ...
    %          \frac{ \overline{ \langle x', y \rangle } }{ | \langle x', y \rangle | }$ \\
    %   &  & & $\langle x, y \rangle \neq 0$ & $\langle x', y \rangle \neq 0$ \\
    %   &  & & $\lambda ( A_{ 1 } x )$ & $\lambda \big( A_{ 1 }( x ) \big)$ \\
    %   &  & & $( a, b ) \in \Omega$ & $( a, b ) \in \Omega$, \\
    %   & &  & $\Ccal^{ 1 }$ & $\Ccal^{ k }$ \\
    %   &  & & $1 / 2 \leq t$ & $1 / 2 < t$ \\
    %   & & & $i_{ x_{ 0 } }( t ) = 0$ & $i_{ x_{ 0 } }( t ) = x_{ 0 }$ \\
    %   &  & & $e_{ i } ( e_{ j }, e_{ k } )$
    %        & $e_{ i } \cdot ( e_{ j } \cdot e_{ k } )$ \\
    %   & & &
    %        & $( e_{ i } \cdot e_{ j } ) \cdot e_{ k }$ \\
    %   & &  & $\bar{q} q$ & $\bar{q} \cdot q$ \\
    %   & &  & $e_{ i } ( e_{ j } \cdot e_{ k } )$
    % & $e_{ i } \cdot ( e_{ j } \cdot e_{ k } )$ \\
    37  &  8 & & $e_{ i } ( e_{ j }, e_{ k } )$
           & $e_{ i } \cdot ( e_{ j } \cdot e_{ k } )$ \\
  ...
  \begin{tabular}{|c|c|c|c|c|}
    ...
    %    & &  & klasa grup & klasa \\
    %    & &  & sumę & \textit{sumę} \\
    %    & &  & $M_{ 1 } \ldots M_{ k }$ & $M_{ 1 }, \ldots, M_{ k }$ \\
    %    & & & $( m,\; n )$ & $( m, n )$ \\
    %    & & & $n \in N$ & $m \in M$ \\
    %    & & & $f( m ) \in M$ & $f( m ) \in N$ \\
    %    & & & $f( n )$ & $f( m )$ \\
    %    & &  & $x_{ 1 },\;\; x_{ 2 } \in X$ & $x_{ 1 },\, x_{ 2 } \in X$ \\
    %    & &  & albo & lub \\
    %   &  & & $y_{ 0 } \succ x$($y_{ 0 } \prec x$)
    %        & $x \prec y_{ 0 }$ ($y_{ 0 } \prec x$) \\
    %   & & & \textit{to istnieje w~$X$} & \textit{to w~$X$ istniej} \\
    %   & &  & $U \subset \Omega_{ x_{ 0 } }$ & $U \in \Omega_{ x_{ 0 } }$ \\
    %   & & & $\rho( X, Y )$ & $\rho( x, y )$ \\
    %   & & & zbioru & niepustego zbioru \\
    %   &  & & $y_{ \varphi } \in X_{ \varphi }$ & $x_{ \varphi } \in X_{ \varphi }$ \\
