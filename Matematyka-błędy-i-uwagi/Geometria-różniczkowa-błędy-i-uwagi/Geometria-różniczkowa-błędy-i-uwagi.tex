...
Z~wzoru \eqref{eq:Wojtynski-09} wynika teraz, że~$x_{ l } = \delta^{ i }_{ l }$,
...
\begin{equation}
  ...
  \delta^{ j }_{ i } \, v_{ j },
\end{equation}
co dowodzi, że~$M( \id ) = [ e_{ 0 } \, \delta^{ j }_{ i } ]$. Przejdźmy do
zbadania $M( f \circ g )$.
\begin{equation}
  \label{eq:Wojtynski-82}
  \begin{split}
...
jako przestrzenie wektorowe nad $\Cbb$. W~$\Cbb^{ 2n }$ określamy standardową bazę

$S : \lFrak( n, \Hbb ) \to \lFrak( 2n, \Cbb )$, które określimy w~sposób bardziej formalny, niż zostało to zrobione w~książce.
...
% Po drugie, jeśli $X$ jest prawą półalgebrą, a~$Y$ jest przestrzenią
% wektorową, jak zwykle nad tym samym działem, która posiada działanie
% wewnętrzne $\cdot_{ Y }$ oraz istnieje izomorfizm liniowy $f : X \to Y$, to $Y$
% jest prawą półalgebrą. Dowód tej własności przebiega w~następujący sposób.
