...
\start \Str{5} Czy podana tu definicja mnożenia w~monoidzie
...
\start \StrWd{7}{15} Użyty w~podanej tu formule $\rho \subset S^{ 2 }$
...
\start \Str{8--9} By uniknąć nieporozumień, należy jasno stwierdzić,
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
$\forall \, a_{ i } \in A$, $i = 1, 2, \ldots, n$. Dla większej przejrzystości
działanie wewnętrzne w~monoidzie $M$ zapisaliśmy tu za pomocą symbolu
$\cdot$. Równania te gwarantują nam, że~może istnieć co najwyżej jedno
odwzorowanie $h : A^{ * } \to M$, dla których są one spełnione.
...
\start \Str{9} W~celu uproszczenia rozważań wprowadzimy teraz
...
\start \Str{9} Niech $M$ będzie monoidem wolnym, $B \subset M$ jego
...
\start \Str{10} W~dowodzie twierdzenia 1.2.2 użyte jest mowa
...
\start \Str{10--11} W~kontekście podanych tu rozważań, należy
...
\start \Str{10} Wzorując~się na przykładzie 1.2.1 wprowadzimy teraz
...
\start \Str{11} W~dowodzie twierdzenia 1.2.4 jest napisane „Jeśli
...
\start \Str{11} W~dowodzie twierdzenia 1.2.4 fakt, że~grupa jedynki
...
\start \StrWd{11}{11} Według mnie zapis $\min\{ k, l \}$ wyglądałoby
...
\start \Str{12} W~dowodzie twierdzenia 1.2.6 panuje podobny bałagan,
...
\start \Str{12} Twierdzenie 1.2.6 powinno być dalej prawdą, gdy
...
\start \StrWg{13}{8} Nie rozumiem czemu w~tej linii wprowadzono
...
\start \Str{13} Dowód twierdzenia~1.3.1 można przedstawić w~prostszy
...
\start \Str{13} Użyte na tej stronie oznaczenie $w M$ jest tradycyjnym
...
\start \Str{13--14} Fragment dowodu twierdzenia 1.3.2, poświęcony implikacji
...
\start \StrWg{14}{12} Odstępy w~tej linii są stanowczo za~duże.
...
\start \StrWd{14}{9} Odstęp nad tą linią jest zbyt duży.
...
\start \StrWd{14}{5} Odstęp nad tą linią jest zbyt duży.
...
\start \Str{16} Warto zauważyć, że~dopuszczamy by ciąg przepisujący słowo
...
\start \Str{16} Oznaczenie „$RS$” pochodzi zapewne od angielskiego
...
\start \Str{16} W~oznaczeniu $L_{ gen }( RS, I )$, „gen” pochodzi zapewne
...
\start \Str{16} W~oznaczeniu $L_{ acc }( RS, I )$, „acc” pochodzi zapewne
...
\start \Str{17} W~tej książeczce brakuje szerszego objaśnienia jaki jest
...
\start \Str{17} Ponieważ w~gramatyce zbiór praw przepisujących~$P$ jest
...
\start \Str{18} Przyjmiemy, że~dla danej gramatyki będziemy używać
...
\start \StrWd{20}{10} W~tym miejscu, tak ja na stronie~7, symbol
...
\start \StrWg{21}{7} W~tej linii znajdujemy tajemniczo wyglądający wzór
\begin{equation}
