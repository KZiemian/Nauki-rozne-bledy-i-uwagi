...
z~wolnym monoidem słownym $A^{ * }$, to można znaleźć taki zbiór
$B \subset M$, że~$M$ jest izomorficzny z~wolnym monoidem słownym
...
Jeśli jednak przez $A^{ * }$ rozumiemy wolny monoid słowny, to jak zostało
...
\end{equation}
Stąd wynika, że~w~takiej sytuacji ciąg \eqref{eq:4} przyjmuje postać
\begin{equation}
  \label{eq:6}
...
$s_{ i } \in S^{ 2 }$ to można go zapisać jako $s_{ i } = t_{ 1 } t_{ 2 }$,
$t_{ 1 }, t_{ 2 } \in S$, więc $m = s_{ 1 } s_{ 2 } \ldots s_{ i - 1 } t_{ 1 } t_{ 2 }
s_{ i + 1 } \ldots s_{ k }$, więc $m \in S^{ k + 1 }$, czyli nie jest prawdą,
iż~$m \notin S^{ k + 1 }$, co kończy dowód.

Oczywiście, jeśli $s_{ i } \in S^{ 2 }$, to istnieje możliwość, że~można go
zasiać jako iloczyn większej liczby elementów, niż 2, choćby
$s_{ i } = t_{ 1 } t_{ 2 } t_{ 3 }$. Dla dowodu omawianej własności nie ma to
jednak znaczenia.
...
$m = s_{ 1 } s_{ 2 } \ldots s_{ k }$, $s_{ i } \in S^{ k } \setminus S^{ k + 1 }$, $i = 1, \ldots, k$.
...
\begin{center}
  ...
  \begin{tabular}{|c|c|c|c|c|}
    ...
    %   &  & & $\forall x, y \in M$ & $\forall x, y \in M,$ \\
    %   & &  & $x \cdot y = 1$ & $x \cdot y = 1_{ M }$ \\
    %   & &  & $\forall x, y, z \in S$ & $\forall x, y, z \in S,$ \\
    %   & &  & $\forall x, y, z \in S$ & $\forall x, y, z \in S,$ \\
    % 7  & &  8 & $\forall x, y, z \in S$ & $\forall x, y, z \in S,$ \\
    % 7  & &  4 & $S / \rho$ & $S_{ / \rho }$ \\
    % 7  & &  3 & $M / \rho$ & $M_{ / \rho }$ \\
  ...
  %   \hline
  %   %  &   & & $\{ 1 \}$ & $\{ 1_{ M } \}$ \\
  %   %  &  & & $\forall n \in \!\! \Nbb$ & $\forall n \in \Nbb$ \\
  %   %  &  & & $( u u^{ -1 } ).....$ & $( u u^{ - 1 } ) \ldots$ \\
  %   %  &  & & $n \in \!\! \Nbb$ & $n \in \Nbb$ \\
  %   %  &  & & $\bigcap_{ n \in \Nbb } \, \mathbf{S}^{ n }$
  %   %        & $\bigcap_{ n \in \Nbb } \, S^{ n }$ \\
  %   %  &  & & \textit{( 1.2.1)} & \textit{(1.2.1)} \\
  %   %  &  & & $S^{ k } \; = 0$ & $S^{ k } = 0$ \\
  %   %  & &  & $\bigcap_{ n \in \Nbb } \mathbf{S}^{ 2 }$
  %   %        & $\bigcap_{ n \in \Nbb } S^{ n }$ \\
  %   %  & &  & $1, ...k$ & $1, \ldots, k$ \\
  %   %  & &  & \textit{polgrupy} & \textit{półgrupy} \\
  %   %  & &  0 & $s_{ 1 }, \hspace{0.5em} s_{ 1 } s_{ 2 },$
  %   %        & $s_{ 1 }, s_{ 2 } s_{ 3 },$ \\
  %   %   &  & & $\forall x \in M$ & $\forall x \in M,$ \\
  %   %   & &  & $( \textrm{\textbf{S}}, \cdot )$ & $( S, \cdot )$ \\
  %   %   & &  & $( \textrm{\textbf{M}}, \cdot, 1_{ \textrm{\textbf{M}}} )$
  %   %        & $( M, \cdot, 1_{ M } )$ \\
  %   %   & &   & $x \in \textrm{\textbf{M}}$ & $x \in M$ \\
  %   %   & &   & $\exists b \in B\; ,$ & $\exists \, b \in B,$ \\
  %   %   &   & & $( S, \cdot )${ }, { }{ }$( S', \ast )$
  %   %        & $( S, \cdot )$, $( S', \ast )$ \\
  %   %   &  & & $\forall x, y \in S$ & $\forall x, y \in S,$ \\
  %   %   &  & & $\forall x, y \in M$ & $\forall x, y \in M,$ \\
  %   %   & &  & $x \cdot y = 1$ & $x \cdot y = 1_{ M }$ \\
  %   %   & &  & $\forall x, y, z \in S$ & $\forall x, y, z \in S,$ \\
  %   %   & &  & $\forall x, y, z \in S$ & $\forall x, y, z \in S,$ \\
  ...
  %   %   &  & & $h : A^{ * } \mapsto M$ & $h : A^{ * } \to M$ \\
  %   %   & &  & $\to \textrm{\textbf{M}}$ & $\to M$ \\
  %   %  & & & $b_{ 1 } ... b_{ k }$ & $b_{ 1 } \ldots b_{ k }$ \\
  %   %  & & & $1, ..., k$ & $1, \ldots, k$ \\
  %   %  &  & & $id_{ A } : A \mapsto M$ & $id_{ A } : A \to M$ \\
  %   %  & &  & $\{ a^{ n } \quad : \quad n \geq 2 \}$ & $\{ a^{ n } \; : \; n \geq 2 \}$ \\
  %   %  &   & & $\{ 1 \}$ & $\{ 1_{ M } \}$ \\
  %   % & & & & \\
  %   % & & & & \\
  %   % & & & & \\
...
\Powin \textit{wolnego monoidu słownego $A^{ * }$} \\
\StrWg{9}{25} \\
\Jest  \textit{homomorfizm} \\
\Powin \textit{homomorfizm z~wolnego monoidu słownego w~dowolny monoid} \\
\StrWd{9}{6} \\
\Jest  \textit{monoidu} $A^{ * }$ \\
