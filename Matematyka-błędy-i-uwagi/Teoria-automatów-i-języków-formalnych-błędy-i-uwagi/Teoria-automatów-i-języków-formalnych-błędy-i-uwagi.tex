...
???? \start \Str{9} Niech $M$ będzie monoidem wolnym, $B \subset M$ jego bazą, a~$h : B^{ * } \to M$ kanonicznym izomorfizmem. Dla $a \in M$ wprowadzamy następującą notację
\begin{equation}
  \label{eq:7}
  \texttt{"} a \texttt{"} :=
...
\start \Str{10--11} W~kontekście podanych tu rozważań, należy zatrzymać~się zatrzymać nad pewnymi własnościami potęg zbiorów w~monoidach i~półgrupach. Niech $S$ będzie dowolną podgrupą, zachodzi wówczas
\begin{equation}
  \label{eq:Forys-Forys-17}
